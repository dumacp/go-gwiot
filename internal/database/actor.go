...
func (a *dbActor) WaitState(ctx actor.Context) {
	...
	case *MsgSearchCollection:
		if err := func() error {
			data := make([]string, 0)
			if err := a.db.View(SearchCollections(&data, msg.Database, []byte(msg.PrefixID), msg.Reverse)); err != nil {

				return err
			}
			if ctx.Sender() != nil {
				ctx.Respond(&MsgSearchCollectionResponse{Database: msg.Database, Collections: data})
			}
			return nil
		}(); err != nil {
			logs.LogError.Println(err)
